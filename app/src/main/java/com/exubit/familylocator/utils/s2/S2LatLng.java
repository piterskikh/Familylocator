...
public class S2LatLng {
    ...
    public boolean isValid() {
        return Math.abs(lat().radians()) <= M_PI_2 && Math.abs(lng().radians()) <= M_PI;
    }
    ...
    public S2LatLng normalized() {
        ...
        return new S2LatLng(Math.max(-M_PI_2, Math.min(M_PI_2, lat().radians())),
                Math.IEEEremainder(lng().radians(), 2 * M_PI));
    }
